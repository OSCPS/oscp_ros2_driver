...
class MagnetometerCalibrationNode(Node):
    ...
    def __init__(self):
        ...
        self.calibrator = MagnetometerCalibrator(
            min_samples=self.target_samples
        )
        ...
        self.get_logger().info(
            "Magnetometer calibration node started"
        )

        self.get_logger().info(
            "Listening on /oscp/raw"
        )

    def goal_callback(self, goal_request):
        self.get_logger().info(
            "Magnetometer calibration request received"
        )
        ...
        if not goal_request.start:
            self.get_logger().warn(
                "Calibration goal requested with start=False"
            )
            return GoalResponse.REJECT
    ...
    def cancel_callback(self, goal_handle):
        self.get_logger().info(
            "Calibration cancellation requested"
        )
    ...
    def raw_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().warn(
                f"Rejected magnetometer sample: {exc}"
            )
        ...
        try:
            while not self.calibrator.ready:
                ...
                feedback.status = (
                    ...
                    f"({sample_count}/{self.target_samples})"
                )
            ...
            self.get_logger().info(
                f"Magnitude min: "
                f"{np.min(magnitudes):.6f}"
            )

            self.get_logger().info(
                f"Magnitude max: "
                f"{np.max(magnitudes):.6f}"
            )

            self.get_logger().info(
                f"Magnitude mean: "
                f"{np.mean(magnitudes):.6f}"
            )

            self.get_logger().info(
                f"Magnitude std: "
                f"{np.std(magnitudes):.6f}"
            )
            ...
            result.hard_iron_x = float(
                calibration.hard_iron[0]
            )

            result.hard_iron_y = float(
                calibration.hard_iron[1]
            )

            result.hard_iron_z = float(
                calibration.hard_iron[2]
            )
            ...
            result.fit_error = float(
                calibration.fit_error
            )
            ...
            self.get_logger().info(
                f"Hard iron: "
                f"[{calibration.hard_iron[0]:.6f}, "
                f"{calibration.hard_iron[1]:.6f}, "
                f"{calibration.hard_iron[2]:.6f}]"
            )
            ...
            self.get_logger().info(
                f"  [{calibration.soft_iron[0, 0]: .6f}, "
                f"{calibration.soft_iron[0, 1]: .6f}, "
                f"{calibration.soft_iron[0, 2]: .6f}]"
            )

            self.get_logger().info(
                f"  [{calibration.soft_iron[1, 0]: .6f}, "
                f"{calibration.soft_iron[1, 1]: .6f}, "
                f"{calibration.soft_iron[1, 2]: .6f}]"
            )

            self.get_logger().info(
                f"  [{calibration.soft_iron[2, 0]: .6f}, "
                f"{calibration.soft_iron[2, 1]: .6f}, "
                f"{calibration.soft_iron[2, 2]: .6f}]"
            )

            self.get_logger().info(
                f"Fit error: "
                f"{calibration.fit_error:.6f}"
            )

            self.get_logger().info(
                f"Mean radius: "
                f"{calibration.mean_radius:.6f}"
            )

            self.get_logger().info(
                f"Std radius: "
                f"{calibration.std_radius:.6f}"
            )
